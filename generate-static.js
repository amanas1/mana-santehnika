...
const faqSchema = {
  ...
  mainEntity: [
    ...
    },
    {
      ...
      acceptedAnswer: { '@type': 'Answer', text: 'Да, на выполненные работы предоставляется гарантия до 12 месяцев в зависимости от вида услуги и установленных материалов.' }
    }
  ]
};
...
seoData.services.forEach(service => {
  ...
  const description = `${service.title} в Алматы: аккуратный выезд мастера, диагностика, честная цена до начала работ, гарантия до 12 месяцев. ${service.description} Звоните: ${DISPLAY_PHONE}`;
...
const blogPages = [
  ...
  { route: 'blog/clog-removal', title: 'Почему засоряется канализация на кухне — советы сантехника', description: 'Почему появляется засор на кухне, как жир забивает трубы и когда нужна профессиональная прочистка канализации в Алматы.', keywords: 'засор канализации алматы, прочистка труб алматы, устранение засора кухня, засорилась канализация' },
